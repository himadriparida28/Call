  ...
  return (
    <div className="page-container max-w-4xl">
      ...
      <motion.div custom={0} variants={fadeUp} initial="hidden" animate="visible" className="card p-6 mb-5">
        ...
        {/* NGO Sahayak Assisted Citizen Filing Banner */}
        {complaint.is_assisted_filing && (
          <div className="mt-4 p-3.5 rounded-xl bg-amber-50/90 border border-amber-200/90 flex items-start gap-3 text-xs text-amber-950">
            <span className="text-xl">🤝</span>
            <div>
              <p className="font-bold text-amber-900 flex items-center gap-2">
                <span>Assisted Citizen Filing via {complaint.assisted_by_ngo_name || 'PRADAN Rural Foundation'}</span>
                <span className="text-[10px] bg-amber-200 text-amber-900 px-1.5 py-0.5 rounded-md font-extrabold uppercase">Grassroots Sahayak</span>
              </p>
              <p className="text-amber-800 mt-1">
                Filing on behalf of: <strong className="text-amber-950">{complaint.beneficiary_name || 'Rameshwar Prasad (Elderly Citizen)'}</strong> · Citizen Mobile: <strong className="font-mono text-amber-950">{complaint.beneficiary_phone || '9876543210'}</strong>
              </p>
            </div>
          </div>
        )}

        {/* 3rd-Party NGO Social Audit Stamp */}
        {complaint.is_ngo_audited && (
          <div className="mt-3 p-3.5 rounded-xl bg-emerald-50 border border-emerald-200 flex items-start gap-3 text-xs text-emerald-950">
            <span className="text-xl">🛡️</span>
            <div>
              <p className="font-bold text-emerald-900 flex items-center gap-2">
                <span>3rd-Party Social Audit Verified by {complaint.ngo_auditor_name || 'Jan Sahas Social Audit Team'}</span>
                <span className="text-[10px] bg-emerald-200 text-emerald-900 px-1.5 py-0.5 rounded-md font-extrabold uppercase">Independent Audit</span>
              </p>
              <p className="text-emerald-800 mt-1">
                {complaint.ngo_audit_remarks || 'On-ground physical verification confirmed by certified NGO social auditor.'}
              </p>
            </div>
          </div>
        )}
