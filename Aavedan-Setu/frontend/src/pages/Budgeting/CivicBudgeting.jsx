            ...
            {filteredProjects.map((proj) => (
              <div key={proj.id} className="card card-hover p-6 flex flex-col justify-between space-y-4 border-2 border-slate-200/80">
                <div>
                  ...
                  {/* 3rd-Party NGO Social Audit & CSR Co-Funding Badges */}
                  <div className="flex flex-wrap items-center gap-2 mt-2.5">
                    {proj.is_ngo_audited && (
                      <span className="text-[10px] font-bold bg-emerald-50 text-emerald-800 border border-emerald-300 px-2 py-0.5 rounded-full flex items-center gap-1 shadow-2xs">
                        <span>🛡️</span> 3rd-Party NGO Audited ({proj.ngo_auditor_name ? proj.ngo_auditor_name.split(" ")[0] + " NGO" : "Jan Sahas NGO"})
                      </span>
                    )}
                    {proj.is_csr_supported && (
                      <span className="text-[10px] font-bold bg-purple-50 text-purple-800 border border-purple-300 px-2 py-0.5 rounded-full flex items-center gap-1 shadow-2xs">
                        <span>🤝</span> CSR Co-Funded: ₹{Number(proj.csr_pledged_amount || 50000).toLocaleString('en-IN')}
                      </span>
                    )}
                  </div>
              ...
              <div className="p-6 overflow-y-auto space-y-6">
                {isLoadingDetail ? (
                  ...
                  <>
                    ...
                    {/* 3rd-Party NGO Social Audit & CSR Co-Funding Ledger */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div className="p-4 bg-emerald-50/80 rounded-2xl border border-emerald-200">
                        <div className="flex items-center gap-2 text-xs font-black text-emerald-900 uppercase mb-1.5">
                          <span className="text-base">🛡️</span>
                          3rd-Party NGO Social Audit Ledger
                        </div>
                        <p className="text-xs text-emerald-950 font-bold">
                          {selectedProject.ngo_auditor_name || 'Jan Sahas Social Audit Team'}
                        </p>
                        <p className="text-[11px] text-emerald-800 mt-1">
                          {selectedProject.ngo_audit_remarks || 'Physical on-ground site inspection verified by certified NGO social auditor.'}
                        </p>
                        <div className="mt-2.5 flex items-center gap-1.5">
                          <span className="text-[10px] bg-emerald-200 text-emerald-900 font-extrabold px-2 py-0.5 rounded-md uppercase">
                      ...
                      <div className="p-4 bg-purple-50/80 rounded-2xl border border-purple-200">
                        <div className="flex items-center gap-2 text-xs font-black text-purple-900 uppercase mb-1.5">
                          <span className="text-base">🤝</span>
                          CSR / Public-Private Co-Funding
                        </div>
                        <p className="text-xs text-purple-950 font-bold">
                          {selectedProject.csr_sponsor_name || 'Tata Trusts & Local CSR Pool'}
                        </p>
                        <p className="text-[11px] text-purple-800 mt-1">
                          Pledged Contribution: <strong className="text-purple-950 font-mono">₹{Number(selectedProject.csr_pledged_amount || 50000).toLocaleString('en-IN')}</strong> matching municipal budget.
                        </p>
                        <div className="mt-2.5 flex items-center gap-1.5">
                          <span className="text-[10px] bg-purple-200 text-purple-900 font-extrabold px-2 py-0.5 rounded-md uppercase">
                            🌟 CSR Grant Active
                          </span>
                        </div>
                      </div>
